...
int main( int argc, char* argv[] ){
    ...
    if (argc != 4) {
        ...
        << " 2. Output file for showered events" << endl
        << " 3. Input parton-level file" << endl
        << " Program stopped. " << endl;
        return 1;
    }
    
    for(int id = 0 ; id < argc ; id++) cout << "Parameter nb " << id << " : " << argv[id] << endl;

    
    Pythia pythia;
    
    // New setting to allow processing of multiple i	nput LHEFs.
    pythia.settings.addMode("LHEFInputs:nSubruns",0,true,false,0,100);
    
    // Input parameters:
    cout << "Will now read the input file" << endl;
    cout << argv[1] << endl;
    cout << argv[2] << endl;
    cout << argv[3] << endl;
    pythia.readFile(argv[1],0);
    pythia.readString("Beams:LHEF = "+string(argv[3]));
    ...
    for (int iMerge = 0; iMerge < nMerge; ++iMerge) {
        ...
        cout << "Read file " << argv[1] << " nb. " << iMerge << " and initialize pythia" << endl;
        ...
        cout << "Determine the inclusive XS" << endl;
        double xs = 0.;
        for (int i=0; i < pythia.info.nProcessesLHEF(); ++i)
        xs += pythia.info.sigmaLHEF(i);
        
        // Start generation loop
        cout << "Begin generation loop" << endl;
